...
fts1 = ftsreader('./20190618/BBIR563_100.0')

npt = fts1.header['Data Parameters IgSm']['NPT']
cb1 = fts1.header['Instrument Parameters']['PKL']
cb2 = npt - fts1.header['Instrument Parameters']['PKL']
phase_ifg_1 = np.zeros(2**17)
phase_ifg_1[0:512] = fts1.ifg[cb1:cb1+512]
phase_ifg_1[-511:] = fts1.ifg[cb1-511:cb1]
...
ifg_1 = np.zeros(2**17)
ifg_1[0:npt/2-cb1] = fts1.ifg[cb1:npt/2]
ifg_1[-cb1:] = fts1.ifg[:cb1]
...
plt.plot(np.flipud(np.array(fts1.spc))/np.max(fts1.spc))
f.show()
